...
def controller_wake_reboot_required(config: AppConfig) -> bool:
    if not config.controller_wake.enabled:
        return False
    configured = config.controller_wake.configured_boot_id
    current = current_boot_id()
    return bool(configured and current and configured == current)
...
def configurable_paths(paths: list[ControllerWakePath] | None = None) -> list[ControllerWakePath]:
    ...
    unique: list[ControllerWakePath] = []
    seen: set[tuple[str, str, str]] = set()
    for item in paths:
        if not USB_ROOT_RE.fullmatch(item.usb_root):
            continue
        key = (item.name, item.usb_root, item.pci_controller)
        if key in seen:
            continue
        seen.add(key)
        unique.append(item)
...
def install_wake_configuration(
    ...
    # The immediate sysfs writes reflect the new setting in the current boot, but
    # onboarding still requires a reboot before controller-wake testing. That gives
    # the persistent udev rule and USB topology a clean initialization boundary.
...
def save_selected_path(config: AppConfig, path: ControllerWakePath, *, settle_delay_seconds: float) -> None:
    ...
    config.controller_wake.configured_boot_id = current_boot_id()
...
def wol_fallback_summary() -> str:
    return (
        ...
        "A controller or wireless dongle must generate a real wake event, the USB root hub and "
        "parent controller must support wake, and BIOS/UEFI must allow that hardware to resume "
        "the machine. Some dongle revisions cannot do it at all. Trying another physical USB port "
        "can help because ports may be attached to different root hubs or USB controllers.\n\n"
        "Wake-on-LAN is a useful fallback for a couch PC. Configure it using the method recommended "
        "for your distribution, network manager, NIC, and firmware, then use a Wake-on-LAN app on "
        "your phone. Once the PC wakes, atv-couch-wake still handles the TV wake and input switch, "
        "so you keep the same console-like effect—the initial wake simply comes from the phone "
        "instead of the controller.\n\n"
        "Wake-on-LAN setup varies enough across Linux distributions and hardware that "
        "atv-couch-wake intentionally does not configure it automatically. Wired Ethernet is the "
        "most broadly supported path; wake over Wi-Fi is a separate hardware-dependent feature.\n\n"
        "Do not expose ADB directly to the public internet. For remote access, use a VPN into your "
        "home network."
    )
